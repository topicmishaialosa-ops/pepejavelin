...
public class StringComponent extends Component implements IMinecraft {
   ...
   private TextFieldWidget field() {
      if (!this.initialized && mc.textRenderer != null) {
         this.field = new TextFieldWidget(mc.textRenderer, 0, 0, 70, 14, Text.empty());
   ...
   public void render(CustomDrawContext draw, Theme theme, float mouseX, float mouseY, float alpha) {
      ...
      float fieldX = this.x + this.width - 78.0F;
      float fieldY = this.y + 2.0F;
      DrawUtil.drawRoundedRect(draw.getMatrices(), fieldX - 1.0F, fieldY - 1.0F, field.getWidth() + 2.0F, field.getHeight() + 2.0F, BorderRadius.all(3.0F), (new ColorRGBA(25, 25, 25)).withAlpha(170.0F * alpha));
      if (field.isFocused()) {
         DrawUtil.drawRoundedBorder(draw.getMatrices(), fieldX - 1.0F, fieldY - 1.0F, field.getWidth() + 2.0F, field.getHeight() + 2.0F, 1.0F, BorderRadius.all(3.0F), theme.getColor().withAlpha(255.0F * alpha));
      }
