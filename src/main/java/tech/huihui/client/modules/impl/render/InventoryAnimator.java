...
@ModuleAnnotation(
   ...
   description = "Анимация предметов и панели в инвентаре"
)
public final class InventoryAnimator extends Module {
   public static final InventoryAnimator INSTANCE = new InventoryAnimator();
   private final BooleanSetting slots = new BooleanSetting("Анимация слотов", true);
   private final BooleanSetting panel = new BooleanSetting("Анимация панели", true);
   ...
   private final NumberSetting panelDuration = new NumberSetting("Длительность панели (мс)", 450.0F, 100.0F, 1500.0F, 25.0F);
   ...
   public float getSlotProgress(int slotId, long now) {
   ...
   public float getPanelProgress(long now) {
      long elapsed = now - this.openTime;
      if (elapsed <= 0L) {
         return 0.0F;
      }
      float p = (float)elapsed / this.panelDuration.getCurrent();
      if (p > 1.0F) {
         return 1.0F;
      }
      return p;
